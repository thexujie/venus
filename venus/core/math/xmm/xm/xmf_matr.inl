...
#else
	float_32 x = matr1[0][0];
	float_32 y = matr1[0][1];
	float_32 z = matr1[0][2];
	float_32 w = matr1[0][3];
...
#else
	float_32 cr = float_cos(rot.x);
	float_32 sr = float_sin(rot.x);
	float_32 cp = float_cos(rot.y);
	float_32 sp = float_sin(rot.y);
	float_32 cy = float_cos(rot.z);
	float_32 sy = float_sin(rot.z);
...
// ÈÆ¹Ì¶¨ÖáÐý×ª
inline xmm xm_matr_rotate_axis_normal(const xmf & axis, float_32 angle)
{
#ifdef VXM_SSE
	float_32 fSinAngle;
	float_32 fCosAngle;
...
inline xmm xm_matr_rotate_axis(const xmf & axis, float_32 angle)
{
#ifdef VXM_SSE
	xmf Normal = xm_vec3_normalize(axis);
	return xm_matr_rotate_axis_normal(Normal, angle);
#else
	float_32 c = float_cos(angle);
	float_32 s = float_sin(angle);
	float_32 t = 1 - c;

	float_32 tx = t * axis.x;
	float_32 ty = t * axis.y;
	float_32 tz = t * axis.z;

	float_32 sx = s * axis.x;
	float_32 sy = s * axis.y;
	float_32 sz = s * axis.z;
...
inline xmm xmm_matr_perspective_lh(float_32 fovy, float_32 aspect, float_32 z_near, float_32 z_far)
{
#ifdef VXM_SSE
	float_32 SinFov;
	float_32 CosFov;
	...
	float_32 fRange = z_far / (z_far - z_near);
	// Note: This is recorded on the stack
	float_32 fHeight = CosFov / SinFov;
...
#else
	xmm result;
	float_32 h = 1 / (float_tan(fovy * 0.5f));
	float_32 w = h / aspect;
	float_32 z = z_far / (z_far - z_near);
	float_32 f = -z_near * z_far / (z_far - z_near);
...
inline xmm xmm_matr_perspective_rh(float_32 fovy, float_32 aspect, float_32 z_near, float_32 z_far)
{
#ifdef VXM_SSE
	float SinFov;
	float CosFov;
	xm_scalar_sin_cos(&SinFov, &CosFov, 0.5f * fovy);
	float fRange = z_far / (z_near - z_far);
	// Note: This is recorded on the stack
	...
	float_32 w = h / aspect;
	float_32 z = z_far / (z_far - z_near);
	float_32 f = z_near * z_far / (z_near - z_far);
	xmm result;
...
inline xmm xm_matr_orthogona_lh(float_32 width, float_32 height, float_32 z_near, float_32 z_far)
{
	xmm result;
...
inline xmm xm_matr_orthogona_rh(float_32 width, float_32 height, float_32 z_near, float_32 z_far)
{
#ifdef VXM_SSE
	float_32 fRange = 1.0f / (z_far - z_near);
	// Note: This is recorded on the stack
...
inline xmf xm_matr_transform_coord(const xmm & matrix, const xmf & vec)
{
	...
	float_32 x = vec.x * row0[0] + vec.y * row1[0] + vec.z * row2[0] + vec.w *row3[0];
	float_32 y = vec.x * row0[1] + vec.y * row1[1] + vec.z * row2[1] + vec.w *row3[1];
	float_32 z = vec.x * row0[2] + vec.y * row1[2] + vec.z * row2[2] + vec.w *row3[2];
