...
{
	float4x4() {}
	float4x4(const float4x4 & another) : row0(another.row0), row1(another.row1), row2(another.row2), row3(another.row3) {}
	float4x4(float4 _row0, float4 _row1, float4 _row2, float4 _row3) :row0(_row0), row1(_row1), row2(_row2), row3(_row3) {}
	float4x4(float4 rows[4]) :row0(rows[0]), row1(rows[1]), row2(rows[2]), row3(rows[3]) {}
	float4x4(float_32 arr[4][4]) :row0(arr[0]), row1(arr[1]), row2(arr[2]), row3(arr[3]) {}
	float4x4(float_32 _00, float_32 _01, float_32 _02, float_32 _03,
			 float_32 _10, float_32 _11, float_32 _12, float_32 _13,
			 float_32 _20, float_32 _21, float_32 _22, float_32 _23,
			 float_32 _30, float_32 _31, float_32 _32, float_32 _33) :
	...
	float4 & operator[](int_x iIndex) { return ((float4 *)this)[iIndex]; }
	const float4 & operator[](int_x iIndex) const { return ((const float4 *)this)[iIndex]; }
	...
	{
		xmm matrix;
		matrix.row0 = row0; matrix.row1 = row1; matrix.row2 = row2; matrix.row3 = row3;
		return matrix;
	}
...
public:
	...
	float4x4 & rotate_axis(const float4 & axis, float_32 angle);

	float4x4 & scale(const float3 & scale);

	float4x4 & perspective_lh(float_32 fovy, float_32 aspect, float_32 z_near, float_32 z_far);
	float4x4 & perspective_rh(float_32 fovy, float_32 aspect, float_32 z_near, float_32 z_far);
...
public:
	union
	{
		...
		struct
		{
			float_32 _11, _12, _13, _14;
			float_32 _21, _22, _23, _24;
			float_32 _31, _32, _33, _34;
			float_32 _41, _42, _43, _44;
		};
	};
};
...
//! 获取一个绕固定轴的旋转矩阵
inline float4x4 float4x4_rotation_axis(const float4 & axis, float_32 angle)
{
...
//! 透视矩阵
inline float4x4 float4x4_perspective_lh(float_32 fovy, float_32 aspect, float_32 z_near, float_32 z_far)
{
	return xmm_matr_perspective_lh(fovy, aspect, z_near, z_far);
}

inline float4x4 float4x4_perspective_rh(float_32 fovy, float_32 aspect, float_32 z_near, float_32 z_far)
{
	return xmm_matr_perspective_rh(fovy, aspect, z_near, z_far);
}

inline float4x4 float4x4_orthogona_lh(float_32 width, float_32 height, float_32 z_near, float_32 z_far)
{
	return xm_matr_orthogona_lh(width, height, z_near, z_far);
}

inline float4x4 float4x4_orthogona_rh(float_32 width, float_32 height, float_32 z_near, float_32 z_far)
{
