...
// 做 2 个字节的测试
bool is_bmp_data(const img_u8 * buffer, img_ix length)
{
	if(length < 3)
		return false;
...
img_ix bmp_create(const img_u8 * buffer, img_ix length, image_data_t * bmp, image_convert_rule_fun_t pfn_match, void * user_data)
{
	if(!pfn_match)
	...
	const img_u8 * conv_palette = nullptr;
	const img_u8 * conv_buffer = nullptr;
	img_ix flags = 0;
	...
	if(header->info_size == sizeof(bmp_info_os2_t))
	{
		...
		width = (img_ix)info->width;
		height = (img_ix)info->height;
		if(height < 0)
			height = -height;
		else
	...
	else
	{
		...
		// 调色板、颜色掩码等额外信息
		img_ix extern_size = header->data_offset - sizeof(bmp_info_windows_t)-sizeof(bmp_header_windows_t);
...
void image_convert_bmp_index4_rle(img_ix width, img_ix height,
								  ...
								  img_ix flags)
{
	...
	const img_u8 * src_color1 = 0;
	const img_u8 * src_color2 = 0;
	const img_u8 * src_pixel = src;
	img_u8 * dst_pixel = dst_line;
	img_u8 * dst_end = dst_pixel + dst_pitch;
	img_ix count = 0;
	bool working = true;
	while(working)
	{
		...
		if(*src_pixel)
		{
			...
			// （特别是最后一行）导致内存访问冲突。。。
			for(img_ix cnt = 0; (cnt < count) && (dst_pixel < dst_end); ++cnt, dst_pixel += dst_stride)
			{
				if(cnt & 1)
		...
		else
		{
			...
			case 2: // 偏移
				...
				{
					img_u8 * skip_end = dst_pixel + count * pitch;
			...
			default: // 连续的、不重复的（Absolute Mode）
				count = *src_pixel++;
				for(img_ix cnt = 0; cnt < count; ++cnt, dst_pixel += dst_stride)
				{
					if(cnt & 1)
					{
...
void image_convert_bmp_index8_rle(img_ix width, img_ix height,
								  pixel_convert_fun_t conv_fun,
								  const img_u8 * pal, img_ix pal_stride,
								  const img_u8 * src, img_ix /*src_stride*/, img_ix /*src_pitch*/,
								  img_u8 * dst, img_ix dst_stride, img_ix dst_pitch,
								  img_ix flags)
{
	...
	const img_u8 * src_color = 0;
	const img_u8 * src_pixel = src;
	img_u8 * dst_pixel = dst_line;
	img_u8 * dst_end = dst_line + dst_pitch;
	img_ix count = 0;
	bool working = true;
	while(working)
	{
		...
		else
		{
			...
			case 2: // 偏移n个像素，该行前*src_index个像素，使用第0个颜色填充
				...
				if(count = *src_pixel++)
				{
					img_u8 * end = dst_pixel + count * pitch;
					while(dst_pixel < end)
					{
						// 换行
						if(dst_pixel >= dst_end)
			...
			default: // 连续的、不重复的
				...
				for(img_ix cnt = 0; cnt < count; ++cnt)
				{
					src_color = pal + (*src_pixel++) * pal_stride;
					conv_fun(src_color, dst_pixel);
					dst_pixel += dst_stride;
