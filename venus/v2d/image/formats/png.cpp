...
#include "../libs/libpng/pngstruct.h"
...
// 做 8 + 12 个字节的测试
bool is_png_data(const img_u8 * pBuffer, img_ix length)
{
	if(length < 21)
		return false;
...
img_ix png_create(const img_u8 * buffer, img_ix length, image_data_t * img,
				image_convert_rule_fun_t pfn_match, void * user_data)
{
	...
	image_convert_rule_t rule = {image_format_png, width, height, src_mode, user_data};
	if(!pfn_match(&rule))
	{
		png_free(png, row_ptr);
		png_destroy_read_struct(&png, &pinfo, 0);
	...
	//png_set_bgr(png);
	img_ix src_pitch = align_to_4((width * src_bits + 7) / 8);
	img_ix src_length = src_pitch * height;

	row_ptr = (img_u8 **)png_malloc(png, height * sizeof(png_bytep));
	img_u8 * src_buffer = image_malloc(src_length);

	img_u8 * line = src_buffer;
